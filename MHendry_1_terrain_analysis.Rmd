...
```{r}
eagle_shed <- tibble(site = c('Eagle River'),
                long = c(39.639286),
                lat = c(-106.572751)) %>%
  st_as_sf(., coords = c('lat','long'), crs = 4263) %>%
  st_transform(2163)
...
#Little code snippet to make the data folder if you don't have it. 
if(!file.exists('eagle_data')){
  dir.create('eagle_data')
}

st_write(eagle_shed, 'eagle_data/pourpoints.shp', append = F)

# Use elevatr to download data and convert to terra object
eagle_elev <- get_elev_raster(eagle_shed, z = 10) %>%
  rast(.)
...
#Save local version for whitebox to use
writeRaster(eagle_elev, 'eagle_data/elev.tif', overwrite = T)

#Get a hillshade map from wbt
wbt_hillshade(dem = "eagle_data/elev.tif",
              output = "eagle_data/hillshade.tif",
              azimuth = 115)

#Read in the hillshade
hillshade2 <- rast('eagle_data/hillshade.tif')
...
# Breach depressions (force water to move through ponds/lakes/etc...)
wbt_breach_depressions_least_cost(
  dem = "eagle_data/elev.tif",
  output = "eagle_data/breached.tif",
  dist = 9,
  fill = TRUE)
...
# Fill any remaining depressions
wbt_fill_depressions_wang_and_liu(
  dem = "eagle_data/breached.tif",
  output = "eagle_data/breachfill.tif"
)

#Get flow accumulation
wbt_d8_flow_accumulation(input = "eagle_data/breachfill.tif",
                         output = "eagle_data/d8fa.tif")
#Get flow direction
wbt_d8_pointer(dem ='eagle_data/breachfill.tif',
               output = 'eagle_data/d8point.tif')

# read in the data
e_fa <- rast('eagle_data/d8fa.tif') %>%
  log10(.)
...
#Extract streams at 1km2 
wbt_extract_streams(flow_accum = "eagle_data/d8fa.tif",
                    output = "eagle_data/raster_streams.tif",
                    threshold = 300)

#Snap our watershed flowliens to this point. 
wbt_jenson_snap_pour_points(pour_pts = "eagle_data/pourpoints.shp",
                            streams = "eagle_data/raster_streams.tif",
                            output = "eagle_data/snappedpp.shp",
                            snap_dist = 200)

# Read in and check the snap. 
e_snap_pour <- st_read('eagle_data/snappedpp.shp')
mapview(e_fa) + 
  mapview(e_snap_pour)

wbt_watershed(d8_pntr = "eagle_data/d8point.tif",
              pour_pts = "eagle_data/snappedpp.shp",
              output = "eagle_data/eagle_river.tif")

e_shed <- rast('eagle_data/eagle_river.tif')
...
wbt_slope(dem = 'eagle_data/elev.tif',
          output = 'eagle_data/slope.tif',
          units = 'degrees')
...
```{r}
# generate ruggedness
wbt_ruggedness_index(dem = 'eagle_data/elev.tif', 
                     output = 'eagle_data/ruggedness.tif')

rugged <- rast('eagle_data/ruggedness.tif')

mapview(rugged)

# Crop rugged to e_shed area

rugged_eagle <- crop(rugged, e_shed) 


rugged_eagle_mask <- mask(rugged, e_shed)


mapview(rugged_eagle_mask)


global(rugged_eagle_mask, fun = 'mean', na.rm = T)


# generate unsphericity

wbt_surface_area_ratio(
  dem = 'eagle_data/elev.tif',
  output = 'eagle_data/surf_area.tif'
)

surf_area <- rast('eagle_data/surf_area.tif')

mapview(surf_area)

# Crop rugged to e_shed area

surf_area_eagle <- crop(surf_area, e_shed) 


surf_area_eagle_mask <- mask(surf_area_eagle, e_shed)


mapview(surf_area_eagle_mask)


global(surf_area_eagle_mask, fun = 'mean', na.rm = T)
